...
const (
	MagicBytes           = 0x4E454F43
	MaxPayloadSize       = 32 * 1024 * 1024
	ProtocolVersion      = 1
	CompressionThreshold = 1024
)

// Codec encodes and decodes P2P messages using protobuf serialization.
type Codec struct {
	useProtobuf    bool
	enableCompress bool
	mu             sync.RWMutex
...
// NewCodec creates a new protobuf codec.
func NewCodec(useProtobuf bool) *Codec {
	return &Codec{
...
func (c *Codec) SetProtobuf(enabled bool) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.useProtobuf = enabled
}

func (c *Codec) SetCompression(enabled bool) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.enableCompress = enabled
}

func (c *Codec) Encode(msgType string, msg interface{}) ([]byte, error) {
	var payload []byte
	var err error
	...
	if compress && len(framed) > CompressionThreshold {
...
func (c *Codec) frame(payload []byte) []byte {
...
func (c *Codec) Decode(r io.Reader) (string, []byte, error) {
	header := make([]byte, 1)
...
// MarshalProtoMessage serializes a protobuf message to bytes.
...
// UnmarshalProtoMessage deserializes bytes to a protobuf message.
