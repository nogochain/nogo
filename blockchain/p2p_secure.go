...
func NewSecureP2PServer(bc *Blockchain, nodeID string, privKey ed25519.PrivateKey, listenAddr string, tlsCert, tlsKey string) (*p2pSecureServer, error) {
	s := &p2pSecureServer{
		bc:         bc,
		nodeID:     nodeID,
		privKey:    privKey,
...
func (s *p2pSecureServer) newHello() p2pSecureHello {
	sigData := fmt.Sprintf("%d|%d|%s|%s|%d", p2pSecureVersion, s.bc.ChainID, s.bc.RulesHashHex(), s.nodeID, time.Now().Unix())
