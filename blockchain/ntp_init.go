...
func initNTPTimeSync(cfg *EnvConfig) error {
	if !cfg.NTPEnabled {
	...
	if cfg.NTPServers != "" {
		...
		parts := strings.FieldsFunc(cfg.NTPServers, func(r rune) bool {
			return r == ',' || r == ' ' || r == ';'
		})
		for _, p := range parts {
	...
	ntp.InitGlobalTimeSync(servers, cfg.NTPSyncInterval, cfg.NTPMaxDrift)
	...
	log.Printf("NTP: time synchronization started (interval=%v, maxDrift=%v)",
		cfg.NTPSyncInterval, cfg.NTPMaxDrift)
