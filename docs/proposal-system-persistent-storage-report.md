...
### Background
The NogoChain community fund governance system requires persistent storage of proposal data to ensure proposal data is not lost after node restart. Additionally, a deposit mechanism needs to be implemented to prevent malicious proposals.

### Objectives
1. ✅ Implement persistent storage of proposal data
...
#### Feature Description
- Proposal data automatically saved to file system
...
- Deposit transactions recorded on blockchain
...
- Ensure transaction is on-chain
...
│  ┌─────────────────────────────────────────────────┐   │
│  │ /api/proposals/deposit   - Create Deposit       │   │
...
4. Miner packs transaction into block
   ↓
5. Frontend polls waiting for transaction confirmation
   ↓
6. Call /api/proposals/create (with depositTx)
   ↓
7. Backend verifies if transaction exists
   ↓
8. Verification passed, create proposal
   ↓
...
// handleCreateProposal (modified version) - verifies deposit transaction
...
#### BlockchainInterface New Methods
```go
...
2. If `depositTx` provided, verify if transaction exists
...
**Response**: Single proposal object (same format as above)
...
```javascript
// Poll to check if transaction is on-chain
...
const proposalResponse = await fetch('/api/proposals/create', {
    ...
    body: JSON.stringify({
        ...
        description: 'Host NogoChain technical seminar',
        type: 'event',
...
#### 1. Persistent Storage
- ✅ Data saved to file after proposal creation
- ✅ Data automatically loaded after node restart
...
#### 3. Transaction Verification
- ✅ HasTransaction method works correctly
- ✅ Able to detect if transaction exists
- ✅ Correctly reports errors on verification failure
...
#### 2. Stress Testing
- ⏳ Concurrent creation of大量 proposals
...
### Testing and Documentation (Priority: Medium)
...
2. **Cross-Node Synchronization**
   - P2P synchronization of proposal data
...
✅ **Persistent Storage**: Proposal data automatically saved, no loss on restart  
✅ **Deposit Mechanism**: Deposit transaction creation and verification  
✅ **API Interfaces**: Complete RESTful API  
✅ **Transaction Verification**: HasTransaction method implemented  
...
```
blockchain/core/contract_manager.go      - ContractManager implementation
blockchain/core/chain.go                 - Chain implementation (added HasTransaction)
blockchain/api/http.go                   - HTTP API implementation
blockchain/network/interfaces.go         - Interface definitions
blockchain/cmd/wrappers.go               - Wrapper implementations
blockchain/contracts/community_fund_governance.go - Contract implementation
```
...
```bash
CONTRACT_DATA_DIR=blockchain_data/contracts  # Persistent data directory
```
...
## 🛡️ Appendix D: Anti-Cheating Mechanism Design
...
```
Early project stage:
- Total circulating: 10,000 NOGO
- Miner A has 51% hashrate → receives 5,100 NOGO
- Quorum 10% → requires 1,000 NOGO

Attack path:
1. Miner A creates proposal: "Give me 5,000 NOGO"
2. Vote yes with their 5,100 NOGO
3. Quorum check: 5,100 > 1,000 ✅ Pass
4. Approval check: 5,100/5,100 = 100% ✅ Pass
5. Result: Legitimately loot community fund ❌

Net gain: 5,000 - 100 (deposit) = 4,900 NOGO
```
...
```
Attacker plan:
- Total circulating: 1,000,000 NOGO
...
#### Scenario 3: Late-Stage Circulation Dispersion (Governance Paralysis)
...
- 20% tokens on exchanges (cannot vote) → 200,000 NOGO locked
- Actual votable: only 30% → 300,000 NOGO
...
- Seems reasonable, but if participation rate is only 5%:
  ...
  - Never reaches quorum ❌ Proposal system paralyzed
```
...
**Core Idea**: Combine advantages of absolute values and relative percentages
...
| Stage | Total Circulating | Absolute Threshold | Relative Threshold (10%) | Actual Requirement | Advantage |
|-------|------------------|-------------------|-------------------------|-------------------|-----------|
| Early | 10,000 | 10,000 | 1,000 | **10,000** | Prevents miner monopoly |
| Mid | 100,000 | 10,000 | 10,000 | **10,000** | Smooth transition |
| Late | 1,000,000 | 10,000 | 100,000 | **100,000** | Adapts to scale |
| Mature | 100,000,000 | 10,000 | 10,000,000 | **10,000,000** | Prevents manipulation |

**Code Implementation**:

```go
// FinalizeVoting - Optimized version (blockchain/contracts/community_fund_governance.go)
func (c *CommunityFundGovernanceContract) FinalizeVoting(proposal_ID string) error {
    c.mu.Lock()
    defer c.mu.Unlock()

    proposal, exists := c.Proposals[proposal_ID]
    if !exists {
...
| Tier | Amount Range | Quorum | Approval | Deposit | Voting Period | Use Cases |
|------|-------------|--------|----------|---------|---------------|-----------|
...
#### Problem: Fixed deposits cannot prevent large-amount cheating
...
| Proposal Amount | Fixed Deposit | Dynamic Deposit | Cheating Cost | Deterrence Effect |
|----------------|---------------|-----------------|---------------|-------------------|
| 100 NOGO | 100 | 100 | 100% | ✅ High |
| 1,000 NOGO | 100 | 200 | 20% | ✅ Medium |
| 10,000 NOGO | 100 | 5,000 | 50% | ✅ Very High |
| 100,000 NOGO | 100 | 50,000 | 50% | ✅ Completely Deterred |
...
// Calculation result: (599 * 100) / 1000 = 59% (precision loss)
// Result: Not passed (but actually very close to 60%)
```

#### Optimization Solution: Use Multiplication Comparison
...
// Left side: 599 * 100 = 59,900
// Right side: 1000 * 60 = 60,000
// 59,900 < 60,000 → Not passed ✅ (Precise)

// VotesFor = 600, Total = 1000, Threshold = 60
// Left side: 600 * 100 = 60,000
// Right side: 1000 * 60 = 60,000
// 60,000 >= 60,000 → Passed ✅ (Precise)
```
...
- [ ] **Voting Approval Rate Optimization**
...
- [ ] **Token Staking System**
  ...
  - [ ] Unlock period restrictions

---

### D.6 Parameter Configuration Recommendations
...
// Result: Quorum insufficient, proposal rejected ✅
```
...
// Result: Even with 500,000 NOGO participation, still doesn't reach 10 million quorum
...
1. **Hybrid Model**: Combine advantages of absolute values and relative percentages
...
// Approval Rate Check (avoid precision loss)
Passed = (VotesFor × 100) >= (TotalVotes × ApprovalThreshold)
```

#### Expected Effects

| Attack Type | Pre-Attack Cost | Post-Attack Cost | Deterrence Effect |
|------------|----------------|-----------------|------------------|
| Miner Monopoly | 100 NOGO | 10,000 NOGO | ✅ 100x increase |
| Sybil Attack | 100 NOGO | 2,000-50,000 NOGO | ✅ 20-500x increase |
| Large-Amount Cheating | 100 NOGO | 50% of proposal amount | ✅ Cost equals benefit |

---

**Report Version**: v2.0 (including anti-cheating plan)  
