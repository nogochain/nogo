...
本文档提�?NogoChain 区块链的完整部署说明，涵盖开发、测试网和生产环境部署。所有内容均基于最新代码实现，确保准确性和可执行性�?
**代码参�?**
...
|---------|-----|------|------|------|---------|
| **最低配�?* | 2 核心 | 2 GB | 10 GB HDD | 10 Mbps | 开�?测试 |
| **推荐配置** | 4 核心 | 8 GB | 100 GB SSD | 100 Mbps | 生产环境 |
| **高性能配置** | 8+ 核心 | 16+ GB | 500+ GB NVMe | 1 Gbps | 高负载场�?|

### 软件要求

- **Go 版本**: 1.21.5（精确版本，禁止使用其他版本�?- **操作系统**: Linux (Ubuntu 20.04+, CentOS 8+), macOS 11+, Windows 10+
- **依赖**: Git, Make（可选）
...
# 或手动下�?wget https://github.com/nogochain/nogo/releases/latest/download/nogo-darwin-amd64
...
| 变量�?| 说明 | 默认�?| 生产建议 |
|--------|------|--------|---------|
| `CHAIN_ID` | �?ID�?=主网�?=测试网，3=烟雾测试�?| `1` | 根据环境选择 |
| `NETWORK_NAME` | 网络名称 | `mainnet` | �?CHAIN_ID 对应 |
| `P2P_PORT` | P2P 网络端口 | `9090` | 开放防火墙 |
| `HTTP_PORT` | HTTP API 端口 | `8080` | 建议绑定 127.0.0.1 |
| `WS_PORT` | WebSocket 端口 | `8081` | 按需开�?|
| `P2P_MAX_PEERS` | 最�?P2P 连接�?| `100` | `100-200` |
| `P2P_MAX_CONNECTIONS` | 最大连接池连接�?| `50` | `50-100` |
| `BOOT_NODES` | 启动节点列表（逗号分隔�?| �?| 配置种子节点 |
| `DNS_DISCOVERY` | DNS 发现服务�?| �?| 配置 DNS 节点 |
...
| `MERKLE_ENABLE` | 启用 Merkle �?| `true` | 必须启用 |
...
| `MAX_REORG_DEPTH` | 最大重组深�?| `100` | 保持不变 |
| `LONG_FORK_THRESHOLD` | 长链分叉阈�?| `10` | 保持不变 |
| `MAX_SYNC_RANGE` | 最大同步范�?| `1000` | 保持不变 |
| `PEER_HEIGHT_POLL_INTERVAL_MS` | 节点高度轮询间隔 | `1000` | 保持不变 |
| `NETWORK_SYNC_CHECK_DELAY_MS` | 网络同步检查延�?| `2000` | 保持不变 |

#### 安全配置

| 变量�?| 说明 | 默认�?| 生产建议 |
|--------|------|--------|---------|
| `ADMIN_TOKEN` | 管理员令牌（最�?16 字符�?| �?| **必须配置** |
| `RATE_LIMIT_REQUESTS` | 每秒请求数限�?| `100` | `50-200` |
| `RATE_LIMIT_BURST` | 请求突发限制 | `50` | `20-100` |
| `TRUST_PROXY` | 信任 X-Forwarded-For �?| `false` | 反向代理时启�?|
| `TLS_ENABLE` | 启用 TLS | `true` | **生产必须启用** |
| `TLS_CERT_FILE` | TLS 证书文件路径 | �?| 配置证书路径 |
| `TLS_KEY_FILE` | TLS 密钥文件路径 | �?| 配置密钥路径 |

#### 交易池配�?
| 变量�?| 说明 | 默认�?| 生产建议 |
|--------|------|--------|---------|
| `MEMPOOL_MAX_SIZE` | 交易池最大交易数 | `10000` | `10000-50000` |
| `MEMPOOL_MIN_FEE_RATE` | 最小手续费�?| `100` | 根据网络调整 |
...
| `STORE_MODE` | 存储模式（pruned/full�?| `pruned` | 生产�?pruned |
...
| `NTP_SERVERS` | NTP 服务器列�?| `pool.ntp.org,time.google.com,time.cloudflare.com` | 保持不变 |
| `NTP_SYNC_INTERVAL_MS` | NTP 同步间隔（毫秒） | `600000`�?0 分钟�?| 保持不变 |
...
| 变量�?| 说明 | 默认�?| 生产建议 |
|--------|------|--------|---------|
| `GOVERNANCE_MIN_QUORUM` | 最小法定人�?| `1000000` | 根据代币分布调整 |
...
| `GOVERNANCE_EXECUTION_DELAY_BLOCKS` | 执行延迟区块�?| `100` | 保持不变 |

#### 功能开�?
| 变量�?| 说明 | 默认�?| 生产建议 |
|--------|------|--------|---------|
| `ENABLE_AI_AUDITOR` | 启用 AI 审计 | `false` | 按需启用 |
| `ENABLE_DNS_REGISTRY` | 启用 DNS 注册�?| `true` | 启用 |
| `ENABLE_GOVERNANCE` | 启用治理 | `true` | 启用 |
| `ENABLE_PRICE_ORACLE` | 启用价格预言�?| `true` | 启用 |
| `ENABLE_SOCIAL_RECOVERY` | 启用社交恢复 | `true` | 启用 |

#### 孤儿池配�?
| 变量�?| 说明 | 默认�?| 生产建议 |
|--------|------|--------|---------|
| `NOGO_ORPHAN_POOL_MAX_SIZE` | 孤儿池最大大�?| `100` | `100-500` |
...
| `NOGO_MINING_SYNC_PAUSE` | 同步时暂停挖�?| `true` | **必须启用** |
...
# 启动 3 节点测试�?docker compose -f docker-compose.testnet.yml up -d

# 查看节点状�?docker compose ps
...
# 查看状�?sudo systemctl status nogochain
...
```bash
# 设置 GOGC 参数（默�?100�?export GOGC=50  # 更频繁的 GC，减少内存使�?export GOGC=200 # 降低 GC 频率，提高性能
    ...
    rules:
      ...
      - alert: NodeDown
        ...
        annotations:
          ...
          description: "节点 {{ $labels.instance }} 已宕机超�?1 分钟"
      ...
      - alert: BlockHeightStuck
        ...
        annotations:
          summary: "区块高度停止增长"
          description: "节点 {{ $labels.instance }} 区块高度�?10 分钟未增�?
      ...
      - alert: HighErrorRate
        ...
        annotations:
          summary: "API 错误率过�?
          description: "错误�?{{ $value | humanizePercentage }} 超过 1%"
      
      # 高延�?      - alert: HighLatency
...
# 3. 检查端口占�?sudo lsof -i :8080
sudo lsof -i :9090

# 4. 检查数据目�?ls -la /var/lib/nogochain
du -sh /var/lib/nogochain/*

# 5. 检查配置文�?./nogo check-config --config config.json

# 6. 测试数据�?./nogo check-db --datadir /var/lib/nogochain
```

**常见问题**:
- 端口被占�?�?更改端口或停止占用进�?- 数据目录权限错误 �?`chown -R nogochain:nogochain /var/lib/nogochain`
- 配置文件错误 �?修复配置
- 数据库损�?�?恢复备份或重建索�?
### 同步缓慢
...
```bash
# 1. 检查网络连�?ping -c 4 main.nogochain.org

# 2. 查看连接节点
curl -s http://localhost:8080/peers | jq '.addresses | length'

# 3. 检查区块高�?curl -s http://localhost:8080/chain/info | jq '.height'
...
# 2. 查看慢查询日�?grep "slow" /var/log/nogochain/nogochain.log | tail -20
...
# 5. 检查并发连�?netstat -an | grep :8080 | wc -l
```

**解决方案**:
- 增加数据库缓�?�?增加 cache_size
- 优化查询 �?使用分页和索�?- 减少并发 �?调整 worker 数量
- 增加资源 �?升级硬件
...
# 复制数据�?cp -r "$NOGO_CHAIN_DIR/data/chain.db" "$BACKUP_DIR/chain_$TIMESTAMP.db"

# 只保留最�?10 个备�?cd "$BACKUP_DIR"
...
# 添加�?5 小时备份一�?0 */5 * * * /path/to/backup.sh >> /var/log/nogochain_backup.log 2>&1
```
...
```bash
# 在线备份（无需停止节点�?cp /var/lib/nogochain/data/chain.db /backup/chain.db.$(date +%Y%m%d)
...
# 备份当前数据（以防万一�?mv /var/lib/nogochain/data /var/lib/nogochain/data.old
...
```bash
# 检查节点状�?curl http://localhost:8080/chain/info

# 检查区块高�?curl http://localhost:8080/chain/info | jq '.height'

# 检查余�?curl http://localhost:8080/account/balance/NOGO0094bc928c08baf466e75fc617f10569a25b1e455caaa421b7f0da239fd5a252b67e070048
```
...
```bash
# 1. 安装新版�?git clone https://github.com/nogochain/nogo.git
cd NogoChain/nogo
go build -o nogo ./blockchain/cmd
...
| 端口 | 协议 | 用�?| 是否对外开�?|
|------|------|------|-------------|
| 8080 | HTTP/TCP | API 服务 | 可�?|
| 8081 | WebSocket | WebSocket 服务 | 可�?|
| 9090 | TCP | P2P 网络 | �?|
| 9100 | TCP | Prometheus 指标 | �?|
...
| `/var/lib/nogochain/keystore/` | 密钥库文�?|
